...
```sh
git clone <this-repo> ~/code/projectionlab-mcp
cd ~/code/projectionlab-mcp
npm install
...
1. **Call `pl_reload_session`** from your MCP client (Claude Code, Claude Desktop, etc.) — closes and re-launches the context against the updated profile. No host restart needed.
2. **Restart the MCP host** (restart the MCP host (e.g., the Claude Code session)). Heavier; only needed if `pl_reload_session` itself is unavailable.
