...
## Scenario flow (what-if comparisons)

For "what if I do X?" questions the user can answer by comparing plans in PL's UI:

1. **Discover** — read the source plan's events with `pl_get_plan_events({ plan_id?, collection? })` to find the ids you'll need for modifications.
2. **Propose** — render the standard confirmation block from `instructions.ts` showing source plan, new name, and each modification's before/after. Wait for a literal `yes`.
3. **Create** — `pl_create_scenario({ source_plan_id?, new_name, modifications? })`. Source defaults to the active plan; `new_name` must be unique. `modifications` is an array of `{ kind: "set_event_fields", collection, match, fields }`. Only `set_event_fields` is supported in v1 — `add_event` / `remove_event` deferred.
4. **Verify** — check `verified: true` in the response and surface any `invariantViolations` loudly. Always relay the `nextStep` line (it points the user at PL → Plans → Compare).
5. **Cleanup** — `pl_delete_scenario({ id })` removes a scenario when the user is done. Look up the id via `pl_export` first; deletion is **irreversible** (snapshot rollback does NOT restore deleted plans).

**Hero use case — mortgage payoff:**
```
pl_create_scenario({
  source_plan_id: "<baseline id>",
  new_name: "Mortgage Paid Off 2026",
  modifications: [{
    kind: "set_event_fields",
    collection: "assets",
    match: { assetId: "<house today-asset id>" },
    fields: { paymentMethod: "cash", balance: 0, monthlyPayment: 0 }
  }]
})
```
Then tell the user to open the new plan in PL UI → Compare against Baseline.

**Limits:**
- Modifications grammar is `set_event_fields` only in v1. Need to add/remove events (e.g., model a one-time payoff expense to get LTCG drag in the projection) — do that in PL UI on the cloned plan.
- Cannot edit `variables`, `withdrawalStrategy`, `montecarlo`, `milestones`, `meta` via modifications — only events under `plan.{income,expenses,assets,accounts,priorities}`. Edit those in PL UI on the clone.
- Structural fields (`id`, `key`, `_meta`, `planPath`, and id-refs `assetId`/`accountId`/`incomeStreamId`/`deductFromIncomeId`/`goalId`/`refId`) are rejected in `fields` — they're regenerated by the clone machinery.
- Snapshot rollback (`pl_restore_apply`) is balance/costBasis only; it does not undo `pl_create_scenario` or `pl_delete_scenario`. Use the corresponding inverse tool.
